...
public class ListElement extends NLGElement {
    ...
    /**
     * Removes the given component to the list element.
     *
     * @param remComponent
     *            the <code>NLGElement</code> component to be removed.
     */
    public void removeComponent(NLGElement remComponent) {
        List<NLGElement> components = getFeatureAsElementList(InternalFeature.COMPONENTS);
        List<NLGElement> toRemove = new ArrayList<NLGElement>();
        if (components != null) {
            for(NLGElement component: components) {
                if(component.equals(remComponent)) {
